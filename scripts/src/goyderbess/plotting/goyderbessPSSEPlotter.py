...
class goyderbessPssePlotter(Plotter): 
    def __init__(
        self, 
...
#-----------------------------------------INV PLOTS----------------------------------------------------------------
        ...
        #scaling = [-200.0,200.0]
        scaling = [138,100]
        axes[0,1].plot(df.index, df["P_REF_MW"]*scaling[0], color=col_pallet[0], label='REF INV1',linestyle="dashed",linewidth=1.5)
        ...
        min_val = min(
            (df["P_REF_MW"] * scaling[0]).min(),
            (df["PMEAS_INV_PU"] * scaling[1]).min()
            )
        max_val = max(
            (df["P_REF_MW"] * scaling[0]).max(),
        ...
        min_val = min( 
            ...
            (df["QMEAS_INV_PU"] * scaling[0]).min(),         
            (df["QMEAS_INV2_PU"] * scaling[2]).min()
            )
        max_val = max( 
            (df["QINV_REF_MVAR"] * scaling[0]).max(),
            (df["QMEAS_INV_PU"] * scaling[0]).max(), 
            (df["QMEAS_INV2_PU"] * scaling[2]).max()
            )
        ax.set_ylim(min_val-2,max_val+2)
        ...
        axes[2,1] .plot(df.index, df["V_GOBESS_PU"], color=col_pallet[0], label='INV1 Vrms',linewidth=1.5)
        ...
        ax = axes[2,1] 
        ax.set_facecolor((0.98,0.98,0.98))
        min_val = (df["V_GOBESS_PU"].min())
        max_val = (df["V_GOBESS_PU"].max())
