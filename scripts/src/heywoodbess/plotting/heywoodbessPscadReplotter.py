"""
This is the class for the new plotter for Heywood BESS - DMAT studies PSCAD
...
class heywoodbessPscadReplotter(Plotter): 
    def __init__(
            self,
...
#------------------- ---------------------------------POC PLOTS -----------------------------------------------------------
        ...
        axes[0,0].plot(df.index, df["PLANT_P_HV"], color=col_pallet[1], label='MEAS',linewidth=1.5)
        ...
        min_val = min(df["Pref_MW"].min(),df["PLANT_P_HV"].min())-1
        max_val = max(df["Pref_MW"].max(),df["PLANT_P_HV"].max())+1
        # ax.set_ylim(min_val-20,max_val+20)


        # [1,0] POC Reactive power
        axes[1,0].plot(df.index, df["Qref_MVAr"], color=col_pallet[0], label='REF',linestyle="dashed",linewidth=1.5)
        axes[1,0].xaxis.set_label_position('top')
        axes[1,0].plot(df.index, df["PLANT_Q_HV"], color=col_pallet[1], label='MEAS',linewidth=1.5)
        ...
        min_val = min(df["Qref_MVAr"].min(),df["PLANT_Q_HV"].min())
        max_val = max(df["Qref_MVAr"].max(),df["PLANT_Q_HV"].max())
        # ax.set_ylim(min_val-20,max_val+20)

        # [2,0] POC Voltage  
        axes[2,0].plot(df.index, df["Vref_pu"], color=col_pallet[0], label='REF',linestyle="dashed",linewidth=1.5)
        axes[2,0].plot(df.index, df["PLANT_V_HV"], color=col_pallet[1], label='MEAS',linewidth=1.5)
        ...
        ax = axes[2,0]
        ax.set_facecolor((0.98,0.98,0.98))   
        min_val = min(df["Vref_pu"].min(),df["PLANT_V_HV"].min())
        max_val = max(df["Vref_pu"].max(),df["PLANT_V_HV"].max())
...
#-----------------------------------------INV PLOTS----------------------------------------------------------------
        ...
        scaling = [4.6,1.0]
        axes[0,1].plot(df.index, df["Pref"]*scaling[0], color=col_pallet[0], label='REF',linestyle="dashed",linewidth=1.5)
        axes[0,1].xaxis.set_label_position('top')
        axes[0,1].plot(df.index, df["PCU2_P_LV"]*scaling[1], color=col_pallet[1], label='MEAS',linewidth=1.5)
        ...
        ax.set_facecolor((0.98,0.98,0.98))
        min_val = min(
            (df["Pref"] * scaling[0]).min(),
            (df["PCU2_P_LV"] * scaling[1]).min()
        )
        max_val = max(
            (df["Pref"] * scaling[0]).max(),
            (df["PCU2_P_LV"] * scaling[1]).max()
        )
        # ax.set_ylim(min_val-20,max_val+20)
        ...
        scaling = [4.6, 1.0]
        axes[1,1].plot(df.index, df["Qref"]*scaling[0], color=col_pallet[0], label='REF',linestyle="dashed",linewidth=1.5)
        axes[1,1].xaxis.set_label_position('top')
        axes[1,1].plot(df.index, df["PCU2_Q_LV"]*scaling[1], color=col_pallet[1], label='MEAS',linewidth=1.5)
        ...
        ax.set_facecolor((0.98,0.98,0.98))
        min_val = min(
            (df["Qref"] * scaling[0]).min(),
            (df["PCU2_Q_LV"] * scaling[1]).min()
        )
        max_val = max(
            (df["Qref"] * scaling[0]).max(),
            (df["PCU2_Q_LV"] * scaling[1]).max()
        )
        # ax.set_ylim(min_val-20,max_val+20)

        # [2,1] INV voltage
        axes[2,1] .plot(df.index, df["VMEAS_LCL"], color=col_pallet[0], label='Vrms',linewidth=1.5)
        ...
        ax = axes[2,1] 
        ax.set_facecolor((0.98,0.98,0.98))
        min_val = df["VMEAS_LCL"].min()
        max_val = df["VMEAS_LCL"].max()
        ...
        axes[2,2].plot(df.index, df["RT_42"], color=col_pallet[0], label='LVRT',linewidth=1.5)
        axes[2,2].xaxis.set_label_position('top')
        axes[2,2].plot(df.index, df["RT_44"], color=col_pallet[1], label='HVRT',linewidth=1.5)
        ...
        ax = axes[2,2] 
        ax.set_facecolor((0.98,0.98,0.98))
        min_val = min(df["RT_42"].min(),df["RT_44"].min())
        max_val = max(df["RT_42"].max(),df["RT_44"].max())
        ax.set_ylim(min_val-2,max_val+2)
